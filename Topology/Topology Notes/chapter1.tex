...
\begin{itemize}
    \item (Discrete metric) : $  \mathcal{T}= \mathcal{P}(X)$
...
\begin{prop}
    ...
    i.e. the set of points such that any open neighborhood of the point intersects $A$. 
\end{prop}

From sheet 1 exercise 9:
An open set $U \in T$ has non-empty intersection with $A$ if and only if $U$ has non-empty intersection with $\overline{A}$
